...
document.addEventListener('DOMContentLoaded', function() {
    ...
    crearGraficoLineas();
    crearGraficoAcordes();
    crearGraficoAcordesBidireccional();
    crearMapaConexiones();
...
function crearGraficoLineas() {
    ...
    d3.json("data/route_class_avg_data_clean.json").then(function(data) {
        ...
        const selectorRuta = d3.select('#contenedor-lineas').append('label')
            .text('Selecciona una Ruta:')
            .append('select')
            .attr('id', 'selectorRuta');
        ...
        const svg = d3.select('#contenedor-lineas').append('svg')
            .attr('id', 'grafico-lineas')
        ...
        // Div del tooltip
        const tooltip = d3.select('#contenedor-lineas').append("div")
            .attr('id', 'tooltip-lineas')
            .attr("class", "tooltip");
...
function crearGraficoAcordes() {
    ...
    d3.json('data/datos_acordes.json').then(function(datosAcordes) {
        const listaCiudades = datosAcordes.ciudades;
        const matrizConexiones = datosAcordes.matriz;

        // Configuración del gráfico de acordes
        const svgAcordes = d3.select('#contenedor-acordes').append('svg')
            .attr('id', 'grafico-acordes')
            .attr('width', 700)
            .attr('height', 700);

        const widthAcordes = +svgAcordes.attr("width");
        const heightAcordes = +svgAcordes.attr("height");
        const innerRadius = Math.min(widthAcordes, heightAcordes) * 0.4;
        ...
        // Crear el gráfico de acordes
        const chords = d3.chord()
            .padAngle(0.05)
            .sortSubgroups(d3.descending)(matrizConexiones);
        ...
        const tooltipAcordes = d3.select('#contenedor-acordes').append("div")
            .attr('id', 'tooltip-acordes')
            .attr("class", "tooltip");
        ...
        gAcordes.datum(chords)
            ...
            .on("mousemove", function(event, d) {
                ...
                tooltipAcordes.style("opacity", 1)
                    .html(`De: ${listaCiudades[d.source.index]}<br>A: ${listaCiudades[d.target.index]}<br>Viajes: ${d.source.value}`)
                    .style("left", (event.clientX - containerRect.left + 10) + "px")
                    .style("top", (event.clientY - containerRect.top - 28) + "px");
            })
            .on("mouseout", function() {
...
// Función para crear el gráfico de acordes bidireccional
function crearGraficoAcordesBidireccional() {
    // Cargar el archivo JSON con los datos para el gráfico de acordes
    d3.json('data/datos_acordes.json').then(function(datosAcordes) {
        const listaCiudades = datosAcordes.ciudades;
        ...
        // Configuración del gráfico de acordes
        const svgAcordes = d3.select('#contenedor-acordes-bidireccional').append('svg')
            .attr('id', 'grafico-acordes-bidireccional')
            .attr('width', 700)
            .attr('height', 700);
        ...
        // Div del tooltip del gráfico de acordes
        const tooltipAcordes = d3.select('#contenedor-acordes-bidireccional').append("div")
            .attr('id', 'tooltip-acordes-bidireccional')
            .attr("class", "tooltip");

        // Obtener la posición del contenedor del gráfico de acordes
        const containerAcordes = d3.select('#contenedor-acordes-bidireccional');
...
function crearMapaConexiones() {
    ...
    // Crear el SVG
    const svg = d3.select('#contenedor-mapa').append('svg')
        .attr('id', 'mapa-espana')
        .attr("width", width)
        .attr("height", height);

    // Crear un elemento para el tooltip
    const tooltip = d3.select('#contenedor-mapa').append("div")
        .attr('id', 'tooltip-mapa')
        .attr("class", "tooltip");
    ...
    Promise.all([
        ...
        d3.json("data/datos_acordes.json") // Cargar datos desde el archivo JSON
...
function crearGraficoFrecuencia() {
    ...
    contenedorFrecuencia.append('h2').text('Frecuencia de Viajes por Hora del Día');

    contenedorFrecuencia.append('label')
        .attr('for', 'sortOrder')
        .text('Ordenar por:');

    const selectOrdenar = contenedorFrecuencia.append('select')
        .attr('id', 'sortOrder');

    selectOrdenar.append('option')
        .attr('value', 'hour')
        .text('Hora del Día');

    selectOrdenar.append('option')
        .attr('value', 'frequency-asc')
        .text('Frecuencia (Ascendente)');

    selectOrdenar.append('option')
        .attr('value', 'frequency-desc')
        .text('Frecuencia (Descendente)');

    // Div para el gráfico
    const divChart = contenedorFrecuencia.append('div')
        .attr('id', 'chart');
    ...
    d3.json("data/travel_frequency.json").then(function(data) {
